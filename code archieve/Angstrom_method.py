...
class Metropolis_Hasting_sampler:
    ...
    def metropolis_hastings_rw(self):
        ...
        # transition_rw_alpha_h = lambda x: np.random.multivariate_normal(x,cov = [[self.transition_sigma[0]**2,-0.5*self.transition_sigma[0]*self.transition_sigma[1]],[-0.5*self.transition_sigma[0]*self.transition_sigma[1],self.transition_sigma[1]**2]])
        ...
        # self.alpha_posterior = np.mean([10**accepted[i,0] for i in range(len(accepted[:,0]))])
        # self.h_posterior = np.mean([10**accepted[i,1] for i in range(len(accepted[:,1]))])
...
df_data = pd.read_csv('rec565_IR_phase_amp_6groups.csv')
exp_setting = {'L':0.025,'f_heating':1,'px':25/10**6,'r':7.62e-5,'cp':385,'rho':8960,'gap':20}
N_sample = 100
params_init = [-4,1.9,-2,-2,0.8] # this is the initial estimate of log(alpha),log(h),log(sigma_dA),log(sigma_dP),rho
prior_log_mu = [-4,2,-2,-2,0.5] # this is the prior belief about the center of the distribution of foregoing parameters
prior_log_sigma = [1,1,1,1,1] # this is the variance used in prior for foregoing parameters
transition_sigma = [0.01,0.01,0.01,0.01,0.02]
result_name = 'Rec565_f1p0_0p4psi_group6_jac_'
#run time 24000 s, 6.6 h
#chain1 = Metropolis_Hasting_sampler(params_init,prior_log_mu,prior_log_sigma,df_data,exp_setting,N_sample,transition_sigma,result_name)

start_time = time.time()
chains = [Metropolis_Hasting_sampler(params_init,prior_log_mu,prior_log_sigma,df_data,exp_setting,N_sample,transition_sigma,result_name) for i in range(4)]
results = Parallel(n_jobs=4)(delayed(chain.metropolis_hastings_rw)() for chain in chains)
...
df_posterior.columns = ['alpha','h','sigma_dA','sigma_dP','corr']
df_posterior['chain_num'] = chain_num
#df_posterior.to_csv(result_name+'MH_'+str(N_sample)+'.csv')
end_time = time.time()
print('run time: '+"{0:.2g}".format(end_time-start_time))
#accept rate 0.03